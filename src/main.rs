...
async fn main() {
    let lines:Vec<Line> = create_rope([0,0],-10,10);
    let points:Vec<&Point> = Line::extract_points(lines);

    loop {
        // update
        simple_force_to_points(points, [0,1], 1);
        simple_sim_step(lines);
        // draw
        clear_background(BLACK);

        for point in points.iter() {
            draw_circle(point.x, point.y, RED);
        }

        for line in lines.iter() {
            draw_line(line.a.x, line.a.y, line.b.x, line.b.y, WHITE);
        }
...
impl Line {
    fn extract_points(lines:Vec<Line>) -> Vec<&Point> {
        let mut out:Vec<&Point> = Vec::with_capacity(lines.len());
        for line in lines {
            if !out.iter().any(|&i| line.a.eq(i)) {
                out.push(line.a)
            }
            if !out.iter().any(|&i| line.b.eq(i)) {
                out.push(line.b)
            }
        }
        out
...
fn simple_forces_to_points(points:Vec<PointAndForce>, delta:f32){
    for i in 0..points.len(){
        let point = &mut points[i].point;
        if point.locked{continue}

        let force = &points[i].force;
...
fn simple_force_to_points(points:Vec<Point>, force:[f32;2], delta:f32) {
    for i in 0..points.len(){
...
fn simple_sim_step(mut lines:Vec<Line>){
...
fn create_rope(start:[f32;2],length:f32,lines:i8) -> Vec<Line> {
    let mut next_pos:[f32;2] = [start[0]+length,start[1]];
    let mut last_point:Point = Point::new(start[0],start[1],true);
    let mut rope:Vec<Line> = Vec::with_capacity(lines as usize);

    for _ in 0..lines {
        let mut current_point = Point::new(next_pos[0], next_pos[1], false);
        next_pos[0] += length;
